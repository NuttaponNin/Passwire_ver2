            ...
            <div class="row">
                <div class="col-lg-9 main-chart">
                    ...
                    <?php                            
                            ...
                            // $cypherKey = mysql_query("SELECT key FROM 'masteraccount' WHERE masterID = '"
                            //         . mysql_real_escape_string($masterID) . "'")
                            //         or die(mysql_error());

                            // $key = openssl_decrypt($cypherKey,"AES-256-CBC",$password, 0, $iv);                

                            $data = mysql_query("SELECT * FROM `storedaccount` WHERE accountID = '"
                                    . mysql_real_escape_string($passDataId) . "'")
                                    or die(mysql_error());
                            while ($info = mysql_fetch_array($data)) {
                              $decrypted_title = openssl_decrypt($info['title'],"AES-256-CBC",$keyCrypto, 0, $iv);
                              $decrypted_username = openssl_decrypt($info['username'],"AES-256-CBC",$keyCrypto, 0, $iv);
                              $decrypted_password = openssl_decrypt($info['password'],"AES-256-CBC",$keyCrypto, 0, $iv);
                              $decrypted_description = openssl_decrypt($info['description'],"AES-256-CBC",$keyCrypto, 0, $iv);
                              ?>
